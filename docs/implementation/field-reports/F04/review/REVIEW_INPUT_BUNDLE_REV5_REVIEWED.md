# F04 ARCHITECTURE REVIEW — INPUT BUNDLE (revision 5)
...
|---|---|---|
| 1 | revision 3: `review/REVIEW_INPUT_BUNDLE_REV3_REVIEWED.md` (sha256 `39bc4a11173c4555…`, 1328 lines) | **ARCHITECTURE_REVIEW_HOLD**: FBR-F04-11 (authorization identity collision); ledger-range drift in WU-04.0 / L-1; H-7 / H-8 reconciliation |
| 2 | revision 4: `review/REVIEW_INPUT_BUNDLE_REV4_REVIEWED.md` (sha256 `378cdf236d4d94bc…`, 1552 lines) | **ARCHITECTURE_REVIEW_HOLD**: FBR-F04-11 **PASS** (operation-scoped authorization model accepted); FBR-F04-12 (retry provenance branches missing from the Inverse DeepSweep; the accepted-artifact identity behind OA-4 was not bound to persisted provenance); current-state drift (ledger 50/41/REC-025; H-7 listed as open; wording implying materialized repairs) |
| 3 | this bundle (revision 5) | pending |

What revision 5 resolved (architecture level; not materialized):
- **FBR-F04-12:** reconstruction §0.1 rule 8. The OA fields and the
  AIOP-002 `precondition_artifact_ref` are written once. Both request
  commands persist their chain reference (BEGIN_ANALYSIS), and the
  clustering request also persists the accepted AIOP-001 artifact. The
  commit-time check verifies persisted references and never substitutes a
  current read. The inverse resolver uses immutable records only.
- **Inverse DeepSweep:** §11 item 2 has branches 2a (OA-1) and 2b (OA-2);
  item 4 has branches 4a (OA-3) and 4b (OA-4).
- **Falsifiers:** E7, E11, E15, K11, K14 and K16 re-evaluated; E16, K17 and
  K18 added.
- **Recursive DeepSweep:** the affected edges were re-run (§10,
  "Revision-5 re-sweep").
- **Documentation drift:** ledger 51 / 42 / REC-027; H-7 removed from the
  inherited open relations (closed as historical drift); NOT_STARTED /
  "not materialized" wording throughout.
- **No new human decision; no new Case 3.** The ledger files 16 and 20 are
  unchanged since revision 4.

## Source files and checksums (sha256 at bundle time)

| Part | Source | Lines | sha256 |
|---|---|---|---|
| full content | `docs/implementation/field-reports/F04/F04_ARCHITECTURE_RECONSTRUCTION.md` | 1050 | `9fcc8e58aee3ff5f…` |
| full content | `docs/implementation/field-reports/F04/HUMAN_DECISIONS.md` | 264 | `eef4115f5844420e…` |
| full content | `docs/implementation/field-reports/F04/STATUS.md` | 66 | `cfff1e1c835f1981…` |
...
**Revision 5 (2026-09-24).** This revision answers the ChatGPT architecture
review of revision 4: **ARCHITECTURE_REVIEW_HOLD**, with FBR-F04-11 PASS and
...
`review/REVIEW_INPUT_BUNDLE_REV3_REVIEWED.md` and
`review/REVIEW_INPUT_BUNDLE_REV4_REVIEWED.md`.
...
- FBR-F04-11 and FBR-F04-12 are resolved at architecture level by
  derivation from HD-16, HD-17 and HD-23. Neither is materialized
...
2. **Visible derived analysis.** Two legal branches (revision 5,
   ...
   - **2b, controller re-run success:** projection ← accepted AIOP-001
     artifact ← acceptance commit (SYSTEM_OPERATION, ref = the request) ←
     BND-010 ← persisted proof VALIDATED ← generation VALIDATED carrying
     **OA-2 = (CMD_REQUEST_QUESTION_ANALYSIS, AIOP-001)**, with `retry_of` →
     the prior FAILED / REJECTED generation ← **CMD_REQUEST_QUESTION_ANALYSIS**
...
3. **"Analysis unavailable"**
   ...
   - The re-run control comes from the server capability (controller,
     R2/R3). A re-run creates OA-2; its success resolves as branch 2b.
   - **Complete.**
4. **Visible clusters (HD-23).** Two legal branches (revision 5,
   ...
   **persisted** `precondition_artifact_ref` (and, for OA-4, the request's
   ...
   - **4b, controller clustering re-run success:** projection ← QuestionCluster
     + memberships ← CMD_ACCEPT_CLUSTERING_OUTPUT (SYSTEM_OPERATION, ref =
     the clustering request) ← BND-010 ← persisted proof VALIDATED ← AIOP-002
     generation VALIDATED carrying **OA-4 =
     (CMD_REQUEST_QUESTION_CLUSTERING, AIOP-002)** and
     `precondition_artifact_ref = X`, with `retry_of` → the prior failed
     AIOP-002 generation (OA-3 or an earlier OA-4) ←
...
| 04.9 | clustering | accepted AIOP-001 artifact + frozen set → ≤1 accepted cluster run | HD-21, HD-23; SYSTEM_OPERATION on OA-3 (ref = C) / OA-4, BINDING (re-run request); 009/010/014 | CMD_AI_QUESTION_CLUSTERING; CMD_ACCEPT_CLUSTERING_OUTPUT; CMD_REQUEST_QUESTION_CLUSTERING; `…/analysis/clustering/request` | `question_clusters`, `question_cluster_memberships` (09 §34/35); uniqueness per OA and per Session; write-once `precondition_artifact_ref` and request references (§0.1 rule 8) | "clustering not run" / clusters | exactly one automatic clustering run (OA-3) after AIOP-001 acceptance, never blocked by the AIOP-001 generation of the same root command; clusters reference only frozen Questions | clustering without an accepted AIOP-001 artifact; AIOP-001 output as input; a second run per OA; a re-run after an accepted cluster run; membership / Question mutation; a priority grant | K1-K18 | 04.5 |
...
- **E7 (re-evaluated in revision 5):** the controller requests a re-run
...
- **E15 (revision 4; re-evaluated in revision 5):** a re-run request
...
- **K16 (revision 4; re-evaluated in revision 5):** a clustering re-run
  after a failed AIOP-002 run carries OA-4. Its chain resolves OA-4 →
...
- every accepted artifact and cluster run is reconstructable to
  ...
  covers the original and re-run branches of both operations (§0.1 rule 8;
  §11 items 2a/2b and 4a/4b);
...
- **Derivable, ready for implementation after review (implementation
  ...
  - FBR-F04-1..9, 11 and 12 each have a defined root repair at their homes.
...
## Status
NOT_STARTED — ARCHITECTURE REVISION 5: READY_FOR_F04_ARCHITECTURE_REVIEW
...
F04_ARCHITECTURE_RECONSTRUCTION.md (revision 5) and HUMAN_DECISIONS.md.
...
**Revision 5 resolves at architecture level:**
...
**First Broken Relations:** twelve.
- FBR-F04-1..9, 11 and 12 have defined root repairs. None is materialized
  (FBR-F04-11 and FBR-F04-12 are resolved at architecture level).
...
All lines are additions (F04 architecture revisions 2-4; unchanged in revision 5); nothing is removed.
================================================================================
