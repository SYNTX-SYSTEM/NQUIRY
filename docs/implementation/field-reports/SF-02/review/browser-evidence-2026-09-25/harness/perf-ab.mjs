// SF-02 D5 reconstruction (WU-SF02.7): cost of the living background under CPU throttling.
...
// After the repair (static rings, bounded opacity-only breathing layer, core glow by opacity, no backdrop-filter):
//   A repaired             16.6 ms · 29.4 ms · 90-166 ms
import { chromium } from "@playwright/test";
const BASE = process.env.BASE ?? "http://localhost:3341";
const API = process.env.API ?? "http://127.0.0.1:13100";
const RATE = Number(process.env.THROTTLE ?? 4);
const VARIANTS = {
  "A full": "",
  "B no background": ".field-bg{display:none!important}",
  "C no drift rings": ".field-bg .drift{animation:none!important}",
  "D no breathe": ".field-bg::after{animation:none!important}",
  "E no backdrop blur": ".shell-header,.plane{backdrop-filter:none!important}",
  "M no core glow anim": ".core::after{animation:none!important}",
  "N no path-pulse": ".orbit-paths .path{animation:none!important}",
};
