/**
 * SF-05 visible-website review harness (evidence tool; not a product test) = the SF-04 harness (53 states: the
 ...
 *   EVIDENCE_RUN=<dir> node docs/implementation/field-reports/SF-05/browser-evidence/harness/review-harness.mjs <workspaceId> <challengeId> [<stressWs> <stressCh>]
 */
...
async function record(p, meta, verify, { expectedHttp = [] } = {}) {
  ...
  results.push(entry);
  writeFileSync(join(ROOT, "results.json"), JSON.stringify({ base: BASE, workspace: WS, challenge: CH, session: SID, generatedAt: new Date().toISOString(), partial: true, results }, null, 2));
  resetLog(log);
  ...
  await record(o, { slug: "workspace-overview-owner", route: "/login -> / -> /workspaces", identity: ID.owner.email, canonicalState: "authenticated; accessible Workspaces from GET /workspaces", expected: "identity/access core; founding relation possible; accessible Workspaces as nodes (links); trace = access only", redirectChain: chain },
    async (page) => {
      ...
      return { projection: { authMe: me.body, workspaces: names }, checks: [
        ...
        check("workspace links = server list (normalized: one node per canonical id, inner + outer ring)", [...new Set(ws.body.workspaces.map((w) => w.workspaceId))].length, await page.locator('[data-testid="workspaces-list"] a, [data-testid="workspaces-list-outer"] a').count(), "projection"),
        check("workspace names = server names (inner ring first)", names.slice(0, 5), (await texts(page.locator('[data-testid="workspaces-list"] a, [data-testid="workspaces-list-outer"] a'))).slice(0, 5), "projection"),
  ...
  await record(o, { slug: "workspace-field-owner", route: "/workspaces/[w]", identity: ID.owner.email, canonicalState: "Workspace; viewer = governance root (Owner), not Facilitator", expected: "workspace core; Challenge relation unavailable with server reason (node + reason); members/authority orbit; add member (governance-capable); proof depth" },
    async (page) => {
      ...
      return { projection: { createChallenge: cc, challenges: ov.body.challenges.map((c) => c.title), members: ov.body.members.map((m) => m.name) }, checks: [
        ...
        check("reason = server reason", cc.reason, cc.available ? null : (await page.getByTestId("challenge-create-unavailable").locator(".boundary-text").textContent()).trim(), "projection"),
...
const grantEvent = await own.page.getByTestId("field-event").textContent().catch(() => null);
await record(own, { slug: "session-grant-committed-owner", route: "/workspaces/[w]/sessions/[s]", identity: ID.owner.email, canonicalState: "SESSION_CONTROL_RIGHT granted at SESSION scope (committed + re-read)", expected: "committed outcome (blue, re-read line), governance node for the holder appears after re-read; no cyan/green success" },
  async (page) => {
    ...
    return { projection: { controllers: pos.body.sessionControllers.map((b) => b.holderName) }, checks: [
      ...
      check("commit resonance after the re-read names the SESSION scope (doc 26 §27, §43.4)", true, typeof grantEvent === "string" && grantEvent.includes("SESSION CONTROL GRANTED") && grantEvent.includes("for this Session") && !grantEvent.includes("for this Challenge")),
      check("the committed outcome stays as a quiet proof line", "true", await page.getByTestId("command-outcome").getAttribute("data-quiet")),
...
await record(fac, { slug: "session-challenge-capture-burst-prepared", route: "/workspaces/[w]/sessions/[s]", identity: ID.facilitator.email, canonicalState: "CHALLENGE_CAPTURE; Burst PREPARED; no participant yet", expected: "lifecycle reconstructed (DRAFT, SETUP passed; CHALLENGE_CAPTURE current); Open question generation BLOCKED with server reason (participant); admit affordance" },
  async (page) => {
    ...
    return { projection: { state: pos.body.session.state, open: pos.body.actions.OPEN_QUESTION_GENERATION }, checks: [
      ...
      check("reason shown when unavailable", pos.body.actions.OPEN_QUESTION_GENERATION.available ? null : pos.body.actions.OPEN_QUESTION_GENERATION.reason, pos.body.actions.OPEN_QUESTION_GENERATION.available ? null : (await page.getByTestId("action-reason-OPEN_QUESTION_GENERATION").locator(".boundary-text").textContent()).trim(), "projection"),
...
// doc 23 §13.6 / §11: the Human Question Field on a wide desktop — input first, governance/proof beside it
{
  ...
  await record(facW, { slug: "human-question-field-wide-constellation", route: "/workspaces/[w]/sessions/[s]", identity: ID.facilitator.email, canonicalState: "QUESTION_GENERATION; Burst ACTIVE HUMAN_ONLY; wide desktop 1600", expected: "instrument constellation: human input in the first column, governance and proof beside it (no waterfall); identity centred; text contained; no node overlaps; lifecycle labels readable" },
    async (page) => { const c = await containment(page); const cols = await instrumentColumns(page); const human = await page.locator('.plane[data-plane="human"]').first().boundingBox(); const gov = await page.locator('.plane[data-plane="governance"]').first().boundingBox(); const proof = await page.locator('.plane[data-plane="proof"]').first().boundingBox(); const labels = await page.locator('[data-testid="session-phases"] .node-label').evaluateAll((els) => els.map((e) => ({ w: e.getBoundingClientRect().width, fs: parseFloat(getComputedStyle(e).fontSize), hidden: e.classList.contains("visually-hidden") })));
...
await record(fac, { slug: "human-question-field-complete-confirm", route: "/workspaces/[w]/sessions/[s]", identity: ID.facilitator.email, canonicalState: "QUESTION_GENERATION; completion requested but NOT committed", expected: "explicit confirmation; still ACTIVE; no frozen topology before commit", keyboard: false },
  async (page) => ({ projection: (await api(fac, `/workspaces/${WS}/sessions/${SID}/position`)).body.burst, checks: [
    ...
    check("confirmation chamber: class, what freezes, what cannot happen, cancel + commit (doc 26 §32)", [true, true, true, 1, 1], [(await page.getByTestId("complete-confirm").getAttribute("data-boundary")) === "IRREVERSIBLE_CONFIRMATION", (await page.getByTestId("complete-confirm").textContent()).includes("becomes immutable"), (await page.getByTestId("complete-confirm").textContent()).includes("cannot happen afterwards"), await page.getByTestId("complete-confirm").getByRole("button", { name: "Keep the Burst open" }).count(), await page.getByTestId("complete-confirm").getByRole("button", { name: "Freeze the set and complete the Burst" }).count()]),
  ] }));
...
// 1–3, 8–9, 13–15: Challenge Field (PRIMARY) on a wide desktop — identity, breadcrumb, background, constellation, containment, reciprocity
{
  ...
  await record(facW, { slug: "challenge-field-wide-constellation", route: "/workspaces/[w]/challenges/[c]", identity: ID.facilitator.email, canonicalState: "Challenge with its Sessions; wide desktop 1600", expected: "centred NQIRY identity; symbiotic breadcrumb (text static, separators breathe); living background across the viewport; expanded spatial distribution; two-column instruments; text contained; no collisions" },
    async (page) => { const c = await containment(page); const rail = await railStable(page); const amb = await ambient(page); const action = await page.locator('.plane[data-plane="action"]').first().boundingBox(); const gov = await page.locator('.plane[data-plane="governance"]').first().boundingBox();
      return { projection: null, checks: [
  ...
  const plane = facW.page.locator('.plane[data-plane="governance"]').first();
...
const organismChecks = (o, { reduced = false } = {}) => [
  ...
  check("chambers are faint membranes inside the organ, never cards (≤ 1 px low-alpha membrane, no outer shadow, translucent fill)", true, (o.organ?.chambers.length ?? 0) > 0 && o.organ.chambers.every((c) => parseFloat(c.borderTop) <= 1 && (c.shadow === "none" || c.shadow.includes("inset")) && parseFloat(c.bg.match(/[\d.]+\)$/)?.[0] ?? "1") < 0.7)),
...
// ================================================================ SF-05 (doc 26): human-review repair states
const organV2 = (page) => page.evaluate(() => {
  const cs = (el) => (el ? getComputedStyle(el) : null);
  const chambers = [...document.querySelectorAll(".organ .plane")];
  const tones = chambers.map((c) => cs(c).getPropertyValue("--chamber-tone").trim());
  const nodes = [...document.querySelectorAll("li.node")];
  const keys = nodes.map((n) => n.dataset.key);
  const metas = nodes.map((n) => n.querySelector(".node-meta")?.textContent ?? "");
  const bodies = nodes.map((n) => n.querySelector(".node-body").getBoundingClientRect());
  let overlaps = 0;
  for (let i = 0; i < bodies.length; i += 1) for (let j = i + 1; j < bodies.length; j += 1) { const a = bodies[i], b = bodies[j]; if (a.left < b.right - 1 && b.left < a.right - 1 && a.top < b.bottom - 1 && b.top < a.bottom - 1) overlaps += 1; }
  const columns = new Set(bodies.map((b) => Math.round((b.left + b.width / 2) / 40))).size;
  const trace = [...document.querySelectorAll('nav[aria-label="Inquiry position"] li')].map((li) => Math.round(li.getBoundingClientRect().top));
  const identity = document.querySelector('[data-testid="identity"]')?.getBoundingClientRect();
  return {
    chambers: chambers.map((c) => c.dataset.semantic), distinctTones: new Set(tones).size,
    authorityChain: [...document.querySelectorAll(".authority-relation")].map((r) => ({ scope: r.dataset.scopeType, roles: [...r.querySelectorAll(".chain-role")].map((x) => x.textContent) })),
    relationMarks: [...document.querySelectorAll(".relation-mark")].map((m) => m.dataset.relationKind),
    spineKinds: [...document.querySelectorAll(".spine-step")].map((x) => x.dataset.kind),
    tokens: document.querySelectorAll(".token").length, tokenWordBreak: cs(document.querySelector(".token"))?.wordBreak ?? null,
    boundaries: [...document.querySelectorAll(".boundary-mark")].map((b) => b.dataset.boundary), boundaryAlerts: document.querySelectorAll('.boundary-mark[role="alert"]').length,
    nodeCount: nodes.length, uniqueKeys: new Set(keys).size, homonymMetas: metas.filter((m) => m.startsWith("Workspace · founded")).length, distinctMetas: new Set(metas).size, overlaps, columns,
    rings: document.querySelectorAll(".orbit").length,
    traceOneLine: new Set(trace).size <= 1, identityCentred: identity ? Math.round(Math.abs(identity.left + identity.width / 2 - window.innerWidth / 2)) : null,
    breathing: [...document.querySelectorAll(".node-aura")].map((a) => cs(a).animationName).every((n) => n === "node-breathe") && new Set([...document.querySelectorAll(".node-aura")].map((a) => cs(a).animationDelay)).size > 1,
    attract: document.querySelector("main.access-field")?.dataset.attract ?? null, focus: document.querySelector("main.access-field")?.dataset.focus ?? null,
    denseOpacity: parseFloat(cs(document.querySelector(".particles-dense .particle"))?.fillOpacity ?? "-1"),
    humanOnlyMarker: document.querySelector('.plane[data-semantic="question"] .chamber-marker')?.textContent ?? null,
    frozen: !!document.querySelector('.frozen-artifact[data-immutable="true"]'),
  };
});
// 54–56: login idle · attention · focus (anonymous, desktop)
{
  const a = await newPage(browser, "desktop");
  await a.page.goto(`${BASE}/login`); await a.page.getByTestId("login-form").waitFor();
  const idle = await organV2(a.page);
  await record(a, { slug: "login-idle", route: "/login", identity: "anonymous", canonicalState: "no session; idle", expected: "calm dark field, readable identity core, quiet ambient particles (doc 26 §28)", keyboard: false },
    async () => ({ projection: null, checks: [check("no attention at rest", [null, null], [idle.attract, idle.focus]), check("dense ambient group quiet at rest", true, idle.denseOpacity >= 0 && idle.denseOpacity < 0.5)] }), { expectedHttp: [/^401 GET \/api\/auth\/me$/, /401 \(Unauthorized\)/] });
  await a.page.getByTestId("login-submit").hover(); await a.page.waitForTimeout(800);
  const hovered = await organV2(a.page);
  await record(a, { slug: "login-attention", route: "/login", identity: "anonymous", canonicalState: "no session; pointer on Log in", expected: "the field gathers: denser/brighter ambient particles, action glow, membrane response; no success implied", keyboard: false },
    async (page) => ({ projection: null, checks: [check("attention recorded as projection-local attribute", "login", hovered.attract), check("dense group gathered", true, hovered.denseOpacity > 0.9), check("action glow", true, (await page.getByTestId("login-submit").evaluate((el) => getComputedStyle(el).boxShadow)) !== "none"), check("core state unchanged (no success)", "current", await page.getByTestId("access-core").getAttribute("data-core-state"))] }), { expectedHttp: [/^401 GET \/api\/auth\/me$/, /401 \(Unauthorized\)/] });
  await a.page.mouse.move(4, 4); await a.page.getByTestId("login-email").focus(); await a.page.waitForTimeout(900);
  const focused = await organV2(a.page);
  await record(a, { slug: "login-focus", route: "/login", identity: "anonymous", canonicalState: "no session; focus in Email", expected: "local chamber responds; the global field stays calm", keyboard: false },
    async () => ({ projection: null, checks: [check("focus recorded locally", "email", focused.focus), check("no global attraction while typing", null, focused.attract), check("dense group quiet again", true, focused.denseOpacity < 0.5)] }), { expectedHttp: [/^401 GET \/api\/auth\/me$/, /401 \(Unauthorized\)/] });
  await a.context.close();
}
// 57–60: Workspace overview with many Workspaces — normalized by canonical id, distributed, at wide / desktop / tablet / phone
for (const vp of ["wide", "desktop", "tablet", "pixel-7"]) {
  const o = await newPage(browser, vp);
  await login(o, ID.owner);
  await o.page.getByTestId("workspaces-list").waitFor(); await o.page.waitForTimeout(700);
  const ws = await api(o, "/workspaces"); const ids = new Set(ws.body.workspaces.map((w) => w.workspaceId)); const names = new Map();
  for (const w of ws.body.workspaces) names.set(w.name, (names.get(w.name) ?? 0) + 1);
  const homonyms = ws.body.workspaces.filter((w) => names.get(w.name) > 1).length;
  const v = await organV2(o.page);
  await record(o, { slug: `workspace-overview-many-${vp}`, route: "/workspaces", identity: ID.owner.email, canonicalState: `${ws.body.workspaces.length} accessible Workspaces (${ids.size} canonical ids, ${homonyms} with a shared name); ${vp}`, expected: "one primary node per canonical id; same-named Workspaces disambiguated by founding time · id; two containment rings using the field; no list, no duplicates, no overlap, no overflow" },
    async (page) => ({ projection: { count: ws.body.workspaces.length, ids: ids.size }, checks: [
      check("one node per canonical id (+ the founding relation)", ids.size + 1, v.nodeCount, "projection"),
      check("no duplicate node keys", v.nodeCount, v.uniqueKeys),
      check("every same-named Workspace carries a distinct canonical distinguisher", homonyms, v.homonymMetas, "projection"),
      check("distinguishers are distinct", true, v.distinctMetas >= homonyms + (ids.size > homonyms ? 1 : 0)),
      check("two containment rings when the collection exceeds the inner ring", ids.size > 5 ? 2 : 1, v.rings),
      check("no node overlaps another", 0, v.overlaps),
      check("nodes spread across the field (desktop and wide ≥ 3 columns — the constellation column is the same width at both; a phone legitimately compresses to one capsule per row)", true, v.columns >= (vp === "pixel-7" ? 1 : 3)),
      check("no horizontal overflow", true, (await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth)) <= 1),
    ] }));
  await o.context.close();
}
// 61–64: Workspace / Challenge fields as owner and facilitator — semantic chambers
for (const [who, url, slug, expectChambers] of [
  [ID.owner, `/workspaces/${WS}`, "workspace-field-owner-chambers", ["action", "identity", "participation", "authority"]],
  [ID.facilitator, `/workspaces/${WS}`, "workspace-field-facilitator-chambers", ["action", "identity", "participation", "authority"]],
  [ID.owner, `/workspaces/${WS}/challenges/${CH}`, "challenge-field-owner-chambers", ["action", "authority", "proof"]],
  [ID.facilitator, `/workspaces/${WS}/challenges/${CH}`, "challenge-field-facilitator-chambers", ["action", "authority", "proof"]],
]) {
  const p = await newPage(browser, "desktop");
  await login(p, who); await p.page.goto(`${BASE}${url}`); await p.page.getByTestId("field-core").waitFor(); await p.page.waitForTimeout(500);
  const v = await organV2(p.page);
  await record(p, { slug, route: url.replace(WS, "[w]").replace(CH, "[c]"), identity: who.email, canonicalState: "same projection as the SF-04 states; organ v2", expected: "differentiated chambers (glyph · title · marker · tone); authority as SOURCE → AUTHORITY → HOLDER → SCOPE; roles as membership marks; proof as a spine; identifiers as tokens; boundaries classed" },
    async () => ({ projection: null, checks: [
      check("semantic chamber classes", expectChambers, v.chambers),
      check("chambers differ in tone", true, v.distinctTones >= 3),
      ...(url.includes("/challenges/") ? [check("authority chain reads source → authority → holder → scope at CHALLENGE scope", true, v.authorityChain.length > 0 && v.authorityChain.every((r) => r.scope === "CHALLENGE" && r.roles.join(",") === "granted by,authority,held by,scope")), check("proof spine present", true, v.spineKinds.length >= 3)] : [check("member roles are membership marks (never authority)", true, v.relationMarks.length > 0 && v.relationMarks.every((m) => ["you", "owner", "facilitator", "contributor", "member"].includes(m)))]),
      check("identifier tokens never break mid-token", ["keep-all", true], [v.tokenWordBreak, v.tokens > 0]),
      check("boundaries are never alerts", 0, v.boundaryAlerts),
    ] }));
  await p.context.close();
}
// 65–67: the frozen Session as facilitator (desktop, phone, reduced motion): question/frozen chambers, participation marks, breathing
if (SID) for (const [vp, rm] of [["desktop", "no-preference"], ["pixel-7", "no-preference"], ["desktop", "reduce"]]) {
  const s = await newPage(browser, vp, { reducedMotion: rm });
  await login(s, ID.facilitator); await s.page.goto(`${BASE}/workspaces/${WS}/sessions/${SID}`); await s.page.getByTestId("field-core").waitFor(); await s.page.waitForTimeout(600);
  const v = await organV2(s.page);
  await record(s, { slug: `session-frozen-chambers-${vp}${rm === "reduce" ? "-reduced" : ""}`, route: "/workspaces/[w]/sessions/[s]", identity: ID.facilitator.email, canonicalState: `QUESTION_CAPTURE; frozen set; ${vp}${rm === "reduce" ? "; reduced motion" : ""}`, expected: "frozen artifact chamber (immutable, verified), authority at SESSION scope, participants with participant/controller marks, provenance spine with commit, decision entry (NON_PROOF)" },
    async (page) => ({ projection: null, checks: [
      check("chambers: frozen · authority · participation · proof · decision-entry", ["frozen", "authority", "participation", "proof", "decision-entry"], v.chambers),
      check("frozen artifact chamber is immutable and verified", [true, "FROZEN"], [v.frozen, (await page.getByTestId("frozen-marker").textContent()).trim()]),
      check("authority at SESSION scope", true, v.authorityChain.length > 0 && v.authorityChain.every((r) => r.scope === "SESSION")),
      check("participation marks distinguish participant and controller", true, v.relationMarks.includes("participant") && v.relationMarks.includes("controller")),
      check("provenance spine: state → actor → authority → commit → time", ["state", "actor", "authority", "commit", "time"], v.spineKinds),
      ...(rm === "reduce" ? [check("reduced motion: no node breathing", true, (await page.locator(".node-aura").first().evaluate((el) => getComputedStyle(el).animationName)) === "none")] : vp === "desktop" ? [check("nodes breathe out of phase", true, v.breathing), check("trace on one line, identity centred", [true, true], [v.traceOneLine, (v.identityCentred ?? 99) < 6])] : []),
    ] }));
  await s.context.close();
}
// 68: the Decision Surface in the field language (facilitator, desktop)
if (SID) {
  const d = await newPage(browser, "desktop");
  await login(d, ID.facilitator); await d.page.goto(`${BASE}/workspaces/${WS}/sessions/${SID}/decision`);
  await d.page.locator('[data-testid="session-view-ok"], [data-testid="denied-banner"], [data-testid="indeterminate-banner"]').first().waitFor(); await d.page.waitForTimeout(500);
  const v = await organV2(d.page);
  await record(d, { slug: "decision-surface-field-language", route: "/workspaces/[w]/sessions/[s]/decision", identity: ID.facilitator.email, canonicalState: "PKG-28/29 Session view (NON_PROOF prototype); same server response as before", expected: "field frame + background + organ chambers around the unchanged components; NON_PROOF explicit; no raw text fallback; no invented chart" },
    async (page) => ({ projection: { ok: await page.getByTestId("session-view-ok").count() }, checks: [
      check("framed in the field", [1, 1, 1], [await page.getByTestId("field-background").count(), await page.getByTestId("context-organ").count(), await page.getByTestId("field-core").count()]),
      check("chambers present", true, v.chambers.length >= 2 && v.chambers.includes("decision-entry")),
      check("NON_PROOF explicit", true, (await page.getByTestId("field-core").textContent()).includes("NON_PROOF")),
      check("no chart/gauge invented", 0, await page.locator("canvas, .chart, .gauge, [data-testid*='chart']").count()),
    ] }));
