...
def _record(
    *,
    ...
    return OutboxRecord(
        ...
        commit_id=commit_id or CommitId(uuid.uuid4()),
...
def test_constructs_a_well_formed_pending_record() -> None:
    record = _record(workspace_id=WorkspaceId(uuid.uuid4()))
...
def test_append_and_get_round_trip(db_connection: sa.Connection) -> None:
    ...
    record = _record(workspace_id=workspace_id)
...
def test_denies_outbox_duplicate(db_connection: sa.Connection) -> None:
    ...
    repo.append(_record(workspace_id=workspace_id, event_id=shared_event_id))
